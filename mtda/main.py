...
class MTDA_FASTAPI(RpcMethodsBase):
    ...
    def load_timeouts_config(self, parser):
        ...
        print(self._session_timeout,"HiIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII\n")
    ...
    async def console_interactive(self):
        if sys.stdin.isatty():
            pass
        self.console_init()
        ...
        prefix_key = None
        if sys.stdin.isatty():
            prefix_key = self.console_prefix_key()
        return True
    ...
    async def target_on(self,session=None):
        ...
        return "Target is ON"
        '''
        print('self.mtda.debug(3, "main.target_on()")')
        self.mtda.debug(3, "I am in main.target_on")
        print(session)
        result = True
        self._session_check(session)
        try:
            with self._power_lock:
                ...
                if status != CONSTS.POWER.ON:
                    print(status,"I am the status of target")
                    result = False
                    #print(self.power_locked(session),"iiiiiiiiiiiii")
                    if self.power_locked(session) is False:
